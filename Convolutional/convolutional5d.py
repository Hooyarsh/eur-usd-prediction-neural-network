...
# Create sequences with only the 5th day ahead as the target
def create_sequences(data, input_len=40):
    X = []
    y = []
    for i in range(len(data) - input_len - 5 + 1):
        X.append(data[i:i + input_len])
        y.append(data[i + input_len + 4])  # Target is the 5th day ahead
...
y_train = torch.FloatTensor(y_train).view(-1, 1)  # Single output (5th day ahead)
...
class CNN2D(nn.Module):
    def __init__(self, input_len):
        ...
        self.fc3 = nn.Linear(64, 1)  # Output 1 value for the 5th day price
    ...
    def forward(self, x):
        ...
        x = self.fc3(x)  # Final output (single value for 5th day)
        return x
...
with torch.no_grad():
    ...
    fifth_day_prediction = cnn2d_model(last_40_days)
    fifth_day_price = scaler.inverse_transform(fifth_day_prediction.numpy())
    print("Predicted price for the 5th day ahead:", fifth_day_price[0][0])