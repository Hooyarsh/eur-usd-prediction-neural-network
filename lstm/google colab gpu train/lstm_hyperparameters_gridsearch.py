...
class MyHyperModel(HyperModel):
    def build(self, hp):
        ...
        model.add(LSTM(
            units=hp.Choice('units', [32, 64, 128]),  # Grid search for these values
        ...
        # Compile the model
        model.compile(
            optimizer=Adam(learning_rate=hp.Choice('learning_rate', [1e-2, 1e-3, 1e-4])),  # Grid search learning rates
            loss='mean_squared_error'
        )
        return model

# Simulate Grid Search using exhaustive search
tuner = Hyperband(
    MyHyperModel(),
    objective='val_loss',  # We want to minimize validation loss
    max_epochs=10,         # Number of epochs for each trial
